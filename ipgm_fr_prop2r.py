...
for i in sorted(curScores, key=lambda x: allDivs.getSortingKeys(x)):
	rData = redressementResults(rData, curScores[i], weight = (1 if i == 'National' else 0.75 if i == 'Province' else 0.5))
...
for i in range(1):
	...
	top2 = sorted(r1s.get('National').result.results, reverse=True, key=lambda x: r1s.get('National').result.results[x])[0:2]
